...
```{r}
City <- get_stamenmap(
    bbox = c(left = -93.60, bottom = 44.71, right = -92.92, top = 45.28), 
    maptype = "terrain",
    zoom = 10)
...
The first line makes "census_pop_est_2018" represent the data set when you use it later in other lines. The second line moves the dot out of the state name. Third line subtracts or removes the dots. The fourth line changes the states from default uppercase to lowercase. The fifth line assigns "starbucks_with_2018_pop_est" to "starbucks_us_by_state". The sixth line uses "left_join" to combine data based on the state. The final line uses the mutate feature to reveal the Starbucks stores per 10000 people.
...
```{r}
pal_line <- colorFactor("viridis", domain = my_favorite_places$top3)
...
```{r}
washington_dc <- get_stamenmap(bbox = c(left = -77.1405, bottom = 38.7813, right = -76.9210, top = 39.0131),
                               maptype = "terrain",
                               zoom = 11)
...
leaflet(mpls_all) %>%
  addTiles() %>%
  addPolygons(
    fillColor = ~pal(proportion_suspicious),
    fillOpacity = .8,
    label = ~BDNAME,
    weight = 1) %>%
  addLegend(pal = pal,
...
```
  ...
  It appears that Southeast neighborhoods such has Morris Park have a higher proportion of suspicisous than any other areas. The northeast has the lowest proportion suspicious. Both north and southwest seem similar, with a diverse array of colors ranging from .8 to .2.
  
  18. Use `leaflet` to create a map of your own choosing. Come up with a question you want to try to answer and use the map to help answer that question. Describe what your map shows.
Where are the poorest neighborhoods located? Is there any correlation to the map above?
```{r}
pal <- colorNumeric("viridis",
                          domain = mpls_all$poverty)
leaflet(mpls_all) %>%
  addTiles() %>%
  addPolygons(fillColor = ~pal_final(poverty),
              fillOpacity = .8,
              label = ~BDNAME,
              weight = 2) %>%
  addLegend(pal = pal,
            values = ~poverty)
```
  Overall, there seems to be some very poor neighbordhoods right a long side rich neighborhoods. I do not see any major similarities between this graph and the one above. Therefore, I cannot claim there is any correlation.
  
## GitHub link

  19. Below, provide a link to your GitHub page with this set of Weekly Exercises. Specifically, if the name of the file is 04_exercises.Rmd, provide a link to the 04_exercises.md file, which is the one that will be most readable on GitHub.

https://github.com/ncamp11/Exercise-4/blob/main/04_exercises.md
